...
<div class="w" id="logo">
    <div>
    	<a href="#">
    		<img src="images/taotao-logo.gif" alt="keyone" width="170" height="60"/>
    	</a> <b></b>
    </div>
</div>
...
<div class="w" id="regist">
    <div class="mt">
        ...
        <div class="extra">
        <span>我已经注册，现在就&nbsp;
        	<a href="http://sso.keyonecn.com/showLogin" class="flk13">登录</a>
        </span>
        </div>
    </div>
...
<script type="text/javascript">
	var REGISTER={
		...
		beforeSubmit:function() {
				//检查用户是否已经被占用
				$.ajax({
	            	url : REGISTER.param.surl + "http://sso.keyonecn.com/user/check/"+escape($("#regName").val())+"/1?r=" + Math.random(),
	            	success : function(data) {
	            		if (data.data) {
	            			//检查手机号是否存在
	            			$.ajax({
	            				url : REGISTER.param.surl + "http://sso.keyonecn.com/user/check/"+$("#phone").val()+"/2?r=" + Math.random(),
				            	success : function(data) {
				            		if (data.data) {
					            		REGISTER.doSubmit();
				            		} else {
		...
		doSubmit:function() {
			$.post("http://sso.keyonecn.com/user/register",$("#personRegForm").serialize(), function(data){
				if(data.status == 200){
					alert('用户注册成功，请登录！');
					REGISTER.login();
				} else {
		...
		login:function() {
			 location.href = "http://sso.keyonecn.com/showLogin";
			 return false;
		},
		reg:function() {
			if (this.inputcheck()) {
